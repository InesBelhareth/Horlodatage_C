...
void evenementLocal() {
    ...
    horloge++;
    printf("[Scalaire %d] Événement local. Horloge = %d\n", idProcessus, horloge);
    pthread_mutex_unlock(&lock);
}

// Prépare le message contenant l'horloge
char* prepareMessage() {
    ...
    // Debug output
    printf("[Scalaire %d] Message préparé: %s\n", idProcessus, buffer);
    pthread_mutex_unlock(&lock);
...
int envoyerMessagePair(int peerId, char* message) {
    ...
    for (int tentative = 1; tentative <= MAX_RETRIES; tentative++) {
        if (connect(sock, (struct sockaddr *)&addr, sizeof(addr)) < 0) {
            printf("[Scalaire %d] Tentative %d échouée vers %d. Réessai...\n", 
...
void envoyerMessage() {
    ...
    for (int i = 0; i < CLIENT_COUNT; i++) {
        if (envoyerMessagePair(i, message) == 0) {
            if (i == idProcessus) {
                printf("[Scalaire %d] Message envoyé à soi-même (horloge = %d)\n", idProcessus, horloge);
            } else {
                printf("[Scalaire %d] Message envoyé à %d (horloge = %d)\n", idProcessus, i, horloge);
            }
        }
    }
}
...
void traiterMessage(char *buffer) {
    // Debugging output
    printf("[Scalaire %d] Message reçu: %s\n", idProcessus, buffer);
    
    int horlogeRecue = atoi(buffer);
    
    pthread_mutex_lock(&lock);
    // Mise à jour correcte de l'horloge scalaire
    horloge = max(horloge, horlogeRecue) + 1;
    printf("[Scalaire %d] Mise à jour horloge après réception: %d\n", idProcessus, horloge);
    pthread_mutex_unlock(&lock);
}
...
void* ecouterConnexions(void *arg) {
    ...
    printf("[Scalaire %d] En écoute sur le port %d\n", idProcessus, BASE_PORT + idProcessus);
